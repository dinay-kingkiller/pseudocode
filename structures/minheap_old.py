class MinHeap:
    ...
    def __init__(self):
        ...
        # queue indices: key = self._queue[self._indices[key]]
        self._indices = {}
    ...
    def __delitem__(self, key):
        # need to clear previous weight in case it increases
    ...
    def __repr__(self):
        extracted_repr = ', '.join(
            [repr(val)+': '+repr(self._values[val]) for val in self._extracted])
        queue_repr = ', '.join(
            [repr(val)+': '+repr(self._values[val]) for val in self._queue])
