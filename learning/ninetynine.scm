...
(define (encode-direct lst)
  (define (encode-tail lst encoded)
    (cond
     ((null? lst) encoded)
     ((null? encoded) (encode-tail (cdr lst) (list (car lst))))
     ((equal? (car lst) (car encoded)) (encode-tail (cdr lst) (cons (list 2 (car encoded)) (cdr encoded))))
     ((not (list? (car encoded))) (encode-tail (cdr lst) (cons (car lst) encoded)))
     ((equal? (car lst) (cadar encoded)) (encode-tail (cdr lst) (cons (cons (+ (caar encoded) 1) (cdar encoded)) (cdr encoded))))
     (else (encode-tail (cdr lst) (cons (car lst) encoded)))))
  (reverse (encode-tail lst '())))
