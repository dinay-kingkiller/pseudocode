...
class KalmanFilter:
    def __init__(self, motion, control, observation, initial_state=None, initial_measurement=None):
        ...
        self.state_error = zeros(len(self.motion), len(self.motion))
        self.measurement_error = zeros(measure_dim, measure_dim)

    def predict(self, input):
        input = matrix(input)
        self.state = self.motion*self.state + self.control*input
        self.state_error = self.motion*self.state_error * \
            transpose(self.motion) + self.noise_error

    def measure(self, measurement):
        measurement = matrix(measurement)
        innovation = measurement - self.observation*self.state
        innovation_error = self.observation*self.covariance * \
            transpose(self.observation) + self.measurement_error
        gain = self.covariance * \
            transpose(self.observation) * inverse(innovation_error)
        self.state = self.state + gain*innovation
        self.state_error = self.state_error - gain * \
            self.observation_error*self.state_error
